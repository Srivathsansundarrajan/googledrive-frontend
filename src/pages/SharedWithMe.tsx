...
export default function SharedWithMe() {
    ...
    const handleOpen = (item: ShareItem) => {
        if (item.resourceType === "file") {
            ...
            // For folders, we would navigate to a shared folder view
                        ...
                        {items.map((item) => (
                            ...
                            >
                                ...
                                <div className="absolute top-2 right-2">
                                    <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${getPermissionBadge(item.permission)}`}>
                                        {item.permission}
                                    </span>
                                </div>

                                <div className="flex justify-center mb-3 mt-2 group-hover:scale-110 transition-transform">
                                ...
                                <div className="mt-2 text-xs text-[var(--text-muted)] text-center truncate" title={`Shared by ${item.sharedBy}`}>
                                    from {item.sharedBy.split("@")[0]}
                                </div>
                            </div>
                        ))}
                    </div>
